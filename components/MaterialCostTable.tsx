...
export const MaterialCostTable: React.FC<MaterialCostTableProps> = ({ rows, updateRow, removeRow, addRow }) => {
    ...
    return (
        <div>
                    ...
                    {rows.map((row, index) => {
                        ...
                        return (
                            <tr key={row.id} className="border-b hover:bg-slate-50">
                                <td className="p-0 border-r"><select value={row.materialId ?? ''} onChange={(e) => updateRow(index, { materialId: e.target.value })} className={`${tableInputClasses} w-full`} required><option value="">Select Material</option>{materials.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></td>
            ...
            <button onClick={addRow} className="mt-3 px-3 py-1 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700">+ Add Material</button>
        </div>
    );
};
